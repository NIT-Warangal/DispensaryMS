	...
	var hovermenu = {
		...
		init: function(opt) {
			...
			$(document).on('keydown', function(event){
				if(event.keyCode==16 && !hovermenu.isOpened) _.openMenu(event);
	...
	var _ = {
		...
		openMenu: function(event){
			...
			hovermenu.menu.css({
				...
				'top' : (hovermenu.currentMouseY-115)+screenTop+'px'
			});
...
$(function(){
	$.hovermenu('init',{
		buttons:{
			"tl":{icon:"fa fa-power-off", text:"Logout", callback: function(){window.open("lock.html","_self"); }},
			"tr":{icon:"fa fa-file-text", text:"Docs", callback: function(){ window.location.href='documentation/#overview' }},
			"bl":{icon:"fa fa-male", text:"Support", callback: function(){window.open("http://bootstrapguru.com/forums/forum/delighted/","_self"); }},
			"br":{icon:"fa fa-envelope", text:"Contact", callback: function(){window.open("http://bootstrapguru.com/contact/","_blank");}}
		}
	});
